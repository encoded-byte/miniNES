...
uint8_t VRC2::read_ram(uint16_t addr)
{
	return ram[addr & 0x1fff];
}

// RAM write: 0x6000 - 0x7fff
void VRC2::write_ram(uint16_t addr, uint8_t data)
{
...
uint8_t VRC2::read_prg(uint16_t addr)
{
...
void VRC2::write_prg(uint16_t addr, uint8_t data)
{
	uint8_t index = (((addr + 0x1000) >> 11) & 0x06) | ((addr >> 1) & 0x01);
	...
	case 0xb002: case 0xc002: case 0xd002: case 0xe002:
		chr_bank[index] = (chr_bank[index] & 0xfff0) | (data & 0x0f);
		break;

	case 0xb001: case 0xc001: case 0xd001: case 0xe001:
	case 0xb003: case 0xc003: case 0xd003: case 0xe003:
		chr_bank[index] = (chr_bank[index] & 0x000f) | (data & 0x1f) << 4;
		break;
	}
}

// PRG write: 0x8000 - 0xffff
void VRC2A::write_prg(uint16_t addr, uint8_t data)
{
	addr = (addr & 0xf000) | ((addr << 1) & 0x02) | ((addr >> 1) & 0x01);
	VRC2::write_prg(addr, data);
}
...
// PRG write: 0x8000 - 0xffff
void VRC4A::write_prg(uint16_t addr, uint8_t data)
{
	addr = (addr & 0xf000) | ((addr << 1) & 0x02) | ((addr >> 1) & 0x01);
	VRC4::write_prg(addr, data);
}

// PRG write: 0x8000 - 0xffff
void VRC4B::write_prg(uint16_t addr, uint8_t data)
{
	addr = (addr & 0xf000) | ((addr >> 1) & 0x02) | ((addr >> 1) & 0x01);
	VRC4::write_prg(addr, data);
}
...
// PRG write: 0x8000 - 0xffff
void VRC4C::write_prg(uint16_t addr, uint8_t data)
{
...
// PRG write: 0x8000 - 0xffff
void VRC4D::write_prg(uint16_t addr, uint8_t data)
{
...
// PRG write: 0x8000 - 0xffff
void VRC4E::write_prg(uint16_t addr, uint8_t data)
{
...
uint8_t VRC2::read_chr(uint16_t addr)
{
...
// CHR read: 0x0000 - 0x1fff
uint8_t VRC2B::read_chr(uint16_t addr)
{
	uint32_t bank_offset = (chr_bank[addr >> 10] >> 1) << 10;
	uint32_t chr_addr = (addr & 0x03ff) | bank_offset;
	chr_addr &= info.chr_size - 1;
...
uint8_t VRC2::read_nt(uint16_t addr)
{
	switch (mirroring)
	{
...
void VRC2::write_nt(uint16_t addr, uint8_t data)
{
	switch (mirroring)
	{
	case 0: addr = addr_nt_vertical(addr); break;
